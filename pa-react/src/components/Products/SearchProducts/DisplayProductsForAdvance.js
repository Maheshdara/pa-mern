...
const DisplayProductsForAdvance = ({
  setVisible,
  searchResults,
  setSideProducts,
  setProduct,
  ...
  const onPageChange = (event) => {
    ...
    setRows(event.rows);
    setAdvancePageNumber(Math.ceil(event.first / event.rows) + 1); // Calculate the page number based on first
};
  ...
  const handlePreviousPage = () => {
    setAdvancePageNumber((prevPage) => Math.max(prevPage - 1, 1)); // Ensure page number doesn't go below 1


  };

  const handleNextPage = () => {
    setAdvancePageNumber((prevPage) => Math.min(prevPage + 1, totalPages)); // Ensure page number doesn't exceed total pages
    ...
    <div>
      ...
      {searchResults.length > 0 && (
          <div style={{ position: "relative", bottom: "0", right: "0", display:"flex",justifyContent:"space-between",marginTop:"10px" }}>
            ...
            onClick={handlePreviousPage} disabled={advancepageNumber === 1}>
              Previous
            </Button>
            <span>
              Page {advancepageNumber} of {totalPages}
            </span>
            <Button
              className="btn btn-outline-success pa-customBtn-sm"
              onClick={handleNextPage}
              disabled={advancepageNumber === totalPages}
            >
              Next
            </Button>
            </div> */}
...
export default DisplayProductsForAdvance;
